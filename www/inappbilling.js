...
InAppBilling.prototype.init = function (success, fail, options, pack, intent, billingKey, skus) {
  options || (options = {});
  ...
  if (!billingKey) {
    if (typeof fail == "function") fail("no billing key");
    if (this.options.showLog) log("no billing key");
    return;
  }
  ...
  if(hasSKUs){
    return cordova.exec(success, fail, "InAppBillingPlugin", "init", [pack, intent, billingKey, skus]);
  }else {
    //No SKUs
    return cordova.exec(success, fail, "InAppBillingPlugin", "init", [pack, intent, billingKey]);
  }
};
